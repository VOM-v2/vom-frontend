...
const SignInPage = () => {
  ...
  const isSigninValid = useMemo(() => {
    return (
      signinForm.signinEmail.trim().length > 0 &&
      signinForm.signinPassword.trim().length > 0 &&
      isEmailLike(signinForm.signinEmail)
    );
  }, [signinForm]);

  const onSigninChange = (field) => (e) => {
    ...
    setSigninErrors((prev) => {
      ...
      if (field === 'signinEmail') {
        if (value.trim() === '') next.signinEmail = '아이디를 입력해주세요';
        else if (!isEmailLike(value)) next.signinEmail = '유효하지 않은 이메일입니다.';
        else delete next.signinEmail;
      }
      if (field === 'signinPassword') {
        if (value.trim() === '') next.signinPassword = '비밀번호를 입력해주세요';
        else delete next.signinPassword;
      }
      return next;
    });
  };
  ...
  const validateSignin = () => {
    const next = {};
    if (!signinForm.signinEmail.trim()) next.signinEmail = '아이디를 입력해주세요';
    else if (!isEmailLike(signinForm.signinEmail))
      next.signinEmail = '유효하지 않은 이메일입니다.';
    if (!signinForm.signinPassword.trim())
      next.signinPassword = '비밀번호를 입력해주세요';
      ...
      <div className="vom-card">
        ...
        <form className="vom-form" onSubmit={onSigninSubmit}>
          ...
          <div className="vom-field">
            <VomInput
              ...
              onChange={onSigninChange('signinPassword')}
              error={signinErrors.signinPassword}
              showPasswordToggle
        ...
        <footer className="vom-footer">
          <div className="vom-signupRow">
            <span className="vom-muted">계정이 없으신가요?</span>
            {/* sign-up naming */}
            <button className="vom-linkBtn" type="button" onClick={() => navigate('/sign-up')}>
              회원가입
            </button>
          </div>
